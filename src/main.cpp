...
#if defined(NDEBUG)
# error "Flocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa5616bc5a2a20d032bc805c8f6348dc832f29465b98e8605585c0affcf15aec7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Flocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Flocoin Signed Message:\n";
...
{
    // Flocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10); // Flocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.007 * 24 * 60 * 60; // Flocoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Flocoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Flocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Flocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "25/01/2018 Naissance du Flocoin !";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04cbe62ca7fe1552a4305abda305f158b4fc711b4e1fb91af7f69ad84f7595266d5cb48914e42cca81776e1f2f0d2ce840fa9a15616a5f18b20dd418bbfad9ea3b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516889759;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087694804;
...
if (false && block.GetHash() != hashGenesisBlock)
{
  ...
  printf("block.GetHash = %s\n", block.GetHash().ToString().c_str());
        ...
        assert(block.hashMerkleRoot == uint256("0xa339c9f6e77a2a73673f6ca820d8f205daccd928b5726c84c3817d580ea1478f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xb1, 0xdb }; // Flocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FlocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FlocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FlocoinMiner(CWallet *pwallet)
{
    printf("FlocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("flocoin-miner");
    ...
    try { loop {
        ...
        printf("Running FlocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FlocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FlocoinMiner, pwallet));
}
